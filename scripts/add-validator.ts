...
// 3. ⚠️ NEW VALIDATOR ADDRESS (DON'T FORGET TO FILL THIS)
...
async function main() {
  console.log("🚀 STARTING ADD VALIDATOR...");
  ...
  console.log(`📦 Payload encoded. Sending transaction to blockchain...`);
  ...
  try {
    ...
    console.log(`🎉 SUCCESS! New validator has been added to the Rollup.`);
  } catch (e: any) {
    console.error("❌ Error:", e);
    // If still error, print raw request for debugging
    if (e.cause) console.error("Root cause:", e.cause);
  }
}
